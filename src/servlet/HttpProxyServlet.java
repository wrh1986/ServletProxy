...
public class HttpProxyServlet extends HttpServlet{
 ...
 //protected boolean doForwardIP = true;
  ...
  public URI getTargetUriObj() {
    if(null != ProxyContext.TARGET_URI_OBJECT) {
      return ProxyContext.TARGET_URI_OBJECT;
    } else {
      return targetUriObj;
    }
  }
  ...
  public String getTargetUri() {
    if(null != ProxyContext.TARGET_URI) {
      return ProxyContext.TARGET_URI;
    } else {
      return targetUri;
    }
  }
  
  public boolean getProxyOrNot() {
    return ProxyContext.PROXY_OR_NOT;
  }
  ...
  public void init(ServletConfig servletConfig) throws ServletException {
    ...
    String doLogStr = servletConfig.getInitParameter(ProxyContext.P_LOG);
    if (doLogStr != null) {
      this.doLog = Boolean.parseBoolean(doLogStr);
    }
    
    try {
      targetUriObj = new URI(servletConfig.getInitParameter(ProxyContext.P_TARGET_URI));
    } catch (Exception e) {
...
* <pre>new DefaultHttpClient(new ThreadSafeClientConnManager(),hcParams)</pre>*/
  ...
  protected void service(HttpServletRequest servletRequest, HttpServletResponse servletResponse)
    ...
    try {
      ...
      proxyResponse = proxyClient.execute(URIUtils.extractHost(this.getTargetUriObj()), proxyRequest);
...
*/
  protected String rewriteUrlFromRequest(HttpServletRequest servletRequest) {
    ...
    if (queryString != null && queryString.length() > 0) {
      ...
      if (this.getProxyOrNot() && fragIdx >= 0) {
        uri.append('#');
...
* and translates it to one the original client can use. */
  protected String rewriteUrlFromResponse(HttpServletRequest servletRequest, String theUrl) {
    ...
    if (theUrl.startsWith(this.getTargetUri())) {
      ...
      theUrl = curUrl+theUrl.substring(this.getTargetUri().length());
    }
    return theUrl;
  }
